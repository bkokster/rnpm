...
export default class PeachMobileComponent extends Component {
    ...
    async submitTransaction(transaction) {
        ...
        if (!transaction) {
            createdTransaction = await PeachMobileComponent.createTransaction(checkoutID, paymentBrand, cardHolder, cardNumber, cardExpiryMonth, cardExpiryYear, cardCVV);
        } else {
        ...
        return PeachMobileComponent.submitTransaction(createdTransaction).then(submitTransactionCompletionHandler);
    }

    static submitTransaction(transaction, mode) {
        if (mode) {
            PeachMobile.initPaymentProvider(mode);
        }
    ...
    async submitRegistration(transaction) {
        const {checkoutID, paymentBrand, cardHolder, cardNumber, cardExpiryMonth, cardExpiryYear, cardCVV} = this.props;
        ...
        const submitRegistrationCompletionHandler = (registrationTransaction) => {
            if (registrationTransaction.transactionType === TransactionTypes.SYNCHRONOUS) {
                this.setState({transactionType: TransactionTypes.SYNCHRONOUS});
                return true;
            }
            if (!_.has(registrationTransaction, 'redirectUrl')) {
                throw Error('Redirect URL is null.');
            }
            this.setState({transactionRedirectUrl: registrationTransaction.redirectUrl, showModal: true});
            return waitForAsynchronousPaymentCallback(500, 5).then(() => {
                this.setState({showModal: false});
                return true;
            }).catch(error => {
        ...
        return PeachMobileComponent.submitRegistration(createdTransaction).then(submitRegistrationCompletionHandler);
    }

    static submitRegistration(transaction, mode) {
        if (mode) {
            PeachMobile.initPaymentProvider(mode);
        }

        return submitRegistration(transaction);
    }
...
function submitRegistration(transaction) {
    return PeachMobile.submitRegistration(transaction);
}
